...
setopt AUTO_CD              # Переход в директорию без явного cd
setopt NO_BEEP              # Отключить бип при ошибках
setopt MULTIOS              # Перенаправление вывода в несколько мест: cmd > file1 > file2
setopt IGNORE_EOF           # Не выходить из zsh по Ctrl+D
setopt LONG_LIST_JOBS       # Подробный формат вывода фоновых задач
