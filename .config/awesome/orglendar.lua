...
local orglendar = { files = {},
                    ...
                    font_task = theme.font or 'monospace 8',
                    parse_on_show = true,
      ...
      if not fd then
         ...
         for line in fd:lines() do
            ...
            if (scheduled and not closed) or (deadline and not closed) then
               local _, _, y, m, d, h, min, recur = string.find(line, "(%d%d%d%d)%-(%d%d)%-(%d%d) [^ ]* ?(%d*)%:?(%d*)[^%+]*%+?([^>]*)>")
               if h ~= "" then
                  h = tonumber(h)
               else
                  h = 23
...
local function create_calendar()
   ...
   return header, format('<span font="%s" foreground="%s">%s</span>',
                         orglendar.font_task, orglendar.text_color, result)
end
...
local function create_todo()
   ...
   return format('<span font="%s" foreground="%s">%s</span>',
                 orglendar.font_task, orglendar.text_color, result), data.maxlen + 3
end
...
function orglendar.get_calendar_and_todo_text(_offset)
   ...
   return format('<span font="%s" foreground="%s">%s</span>\n%s',
                 orglendar.font_task, orglendar.text_color, header, cal), create_todo()
end
